...
from ..utils.helpers import format_bytes, format_speed, get_file_icon, get_file_extension
...
class DownloadCard(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        self.name_label = ctk.CTkLabel(
            self, 
            text=Path(self.task.url).name[:45] if self.task.url else "...",
    ...
    def update_buttons(self):
        ...
        if self.task.status == "DOWNLOADING":
            ctk.CTkButton(
                ...
                fg_color="#ffc107", hover_color="#e6ac00", text_color="black",
                command=lambda: downloader.pause_task(self.task.id)
            ).pack(pady=2)
            
            ctk.CTkButton(
                self.btn_frame, text="✗", width=35, height=30,
                fg_color="#ff4757", hover_color="#ff6b6b",
            ...
            ctk.CTkButton(
                self.btn_frame, text="▶", width=35, height=30,
                fg_color="#00d9a5", hover_color="#00f5b8", text_color="black",
...
class DownloadManagerGUI:
    ...
    def create_sidebar(self):
        ...
        self.completed_label = ctk.CTkLabel(
            self.stats_frame, text="Completadas: 0", font=("Segoe UI", 12), text_color="#00d9a5"
        )
        self.completed_label.pack()
        
        self.failed_label = ctk.CTkLabel(
            self.stats_frame, text="Fallidas: 0", font=("Segoe UI", 12), text_color="#ff4757"
        )
        self.failed_label.pack()
        
        ctk.CTkButton(
            sidebar, text="⚙ Configuración",
    ...
    def create_header(self):
        ...
        self.url_entry = ctk.CTkEntry(
            header, placeholder_text="https://ejemplo.com/archivo.zip",
            height=35
        )
        self.url_entry.grid(row=1, column=1, padx=5, sticky="ew")
        
        ctk.CTkButton(
            header, text="📁", width=40, height=35,
            fg_color="#1f2937", hover_color="#374151",
            command=self.browse_folder
        ).grid(row=1, column=2, padx=(5, 10))
        
        self.dest_entry = ctk.CTkEntry(
            header, placeholder_text="Carpeta de destino",
            height=35
        )
        self.dest_entry.grid(row=2, column=0, columnspan=2, padx=10, pady=(5, 0), sticky="ew")
        self.dest_entry.insert(0, self.config["default_download_path"])
        
        ctk.CTkButton(
            header, text="⬇ DESCARGAR",
            fg_color="#e94560", hover_color="#ff6b6b", height=40,
    ...
    def open_config(self):
        ...
        ctk.CTkLabel(frame, text="⚙ Configuración", font=("Segoe UI", 16, "bold")).pack(pady=(0, 20))
        ...
        ctk.CTkButton(frame, text="💾 GUARDAR", fg_color="#00d9a5", text_color="black",
                     command=save).pack(pady=20, fill="x")
