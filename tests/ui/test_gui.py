...
Usa el servidor Flask simple (tests/api/server.py) en puerto 8000.
...
def test_page_loads_correctly(page: Page, gui_server):
...
def test_app_title_visible(page: Page, gui_server):
...
def test_url_form_elements_exist(page: Page, gui_server):
...
def test_valid_url_shows_success_message(page: Page, gui_server):
...
def test_invalid_url_shows_error_message(page: Page, gui_server):
    """CP-UI-05: URL invalida muestra mensaje de error."""
    page.goto(BASE_URL)
    page.fill("#url-input", "esto-no-es-una-url")
    page.click("#validate-btn")
    result = page.locator("#validation-result")
    expect(result).to_contain_text("URL invalida")


def test_stats_section_loads_with_data(page: Page, gui_server):
